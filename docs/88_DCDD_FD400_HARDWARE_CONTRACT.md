...
- the Disk Buffer's approximately five-second inhibit following power-on or door close has not expired.

Those are physical conditions, not host UI special cases. Phase 3 represents the five-second condition as a virtual-time deadline. A previously latched selection request can become effective when that deadline expires without CPU polling; no per-T-state drive loop is required.
...
| D1 | Move Head | source-backed Phase-3 mechanical waveform indicating when another head movement is permitted; later write/trim-erase phases may additionally suppress it where documented |
...
| D6 | TRACK 0 | true when the head is at the outermost track; Phase 3 derives it from the same FD-400 track state used by stepping |
...
Phase 3 routes D0..D3 over the real Board-1/Board-2 harness to the selected FD-400 mechanics. Conflicting simultaneous step commands or simultaneous load/unload commands do not invent an ordering. IE/ID/HCS/WRITE ENABLE remain deferred to later phases.
...
- `D6..D7`: source review for Phase 3 resolves the valid-read table values as driven logic `1`.

When Head Status is false the sector-position line drivers remain disabled/high impedance. When valid, Phase 3 derives Sector True and the binary sector number analytically from the same 360-RPM virtual-time epoch; polling the port does not advance the disk.
...
Source-backed mechanics implemented in Phase 3:

- track-to-track access is 10 ms;
- tracks are limited to `0..76`;
...
- after initial head load, Move Head remains false until the 40 ms load deadline then becomes true;
- after an accepted step command, Move Head is false for the first 10 ms, true from 10 ms to 11 ms, false from 11 ms to 31 ms, and true again from 31 ms onward;
- a pending step completes at 10 ms and a second step is not accepted while the pending step/Move Head gating says movement is unavailable.

Later write/trim-erase phases may add the additional source-backed Move Head suppression associated with those circuits; Phase 3 does not fabricate write-state effects before the write engine exists.
...
### Phase 2 decode/register boundary — complete

Phase 2 made only the no-media register/electrical surface guest-visible:
...
- no interrupt assertion, media access, read cadence or write cadence.

The fitted DCDD boards do not subscribe to the 2 MHz connector clock merely to poll an idle controller.

### Phase 3 mechanics/time boundary — complete

Phase 3 adds only source-backed mechanics and their controller-visible status:

- one O(1) controller/drive virtual-time epoch advanced from guest T-states;
- 360-RPM rotational phase and hard-sector position derived arithmetically;
- the five-second Disk Buffer selection inhibit;
- drive power, door, motor, track, head-load, Head Status and Move Head deadlines;
- 10 ms track stepping with track limits and 40 ms Head Status behavior;
- TRACK0/Move Head/Head Status through `IN 08h`;
- Head-Status-gated sector/Sector True through `IN 09h`, including valid D6/D7 high;
- Adaptive Full deferred chassis-time advancement and exact Partial advancement over the same epoch;
- no media bytes, read electronics, write electronics or interrupt assertion.
