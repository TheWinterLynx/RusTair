...
fn two_sio_input_adds_one_t_state_on_adaptive_cycle() {
    ...
    assert_eq!(
        after.pc, 0x0002,
        "Adaptive Cycle must complete exactly one IN"
    );
    assert_eq!(
...
fn cycle_88_2sio_input_exposes_one_real_tw_and_releases_prdy_in_tw() {
    ...
    assert_eq!(
        input_samples.len(),
        4,
        "one 88-2SIO IN must contain exactly one TW"
    );
    assert_eq!(
        input_samples
            .iter()
    ...
    assert_eq!(
        t2.ready,
        Some(false),
        "SINP/V must pull PRDY low at the T2 READY sample"
    );
    assert_eq!(
        tw.ready,
        Some(true),
        "PWAIT clears V so PRDY is released during the sole TW"
    );
    assert_eq!(
        tw.pins.wait,
        Some(true),
        "the 8080 must expose a real WAIT/TW output"
    );
    ...
    assert_eq!(
        t3.cpu_data,
        Some(0x02),
        "empty MC6850 status currently reports TDRE"
    );
    assert_eq!(
        t3.s100_di, None,
        "the 88-2SIO must release DI once DBIN ends in T3"
    );
