fn source_contains_ignoring_whitespace(source: &str, expected: &str) -> bool {
    let mut source_chars = source.chars().filter(|ch| !ch.is_whitespace());
    ...
    let source = source_chars.by_ref().collect::<String>();
...
fn intel_8080a_dip40_pinout_keeps_reference_numbering() {
    ...
    for expected in [
        ...
        assert!(
            source_contains_ignoring_whitespace(source, expected),
            "missing reference DIP-40 mapping {expected}"
        );
    }
}
...
fn pin_visualizer_distinguishes_level_assertion_power_and_exact_clock_lines() {
    ...
    assert!(source_contains_ignoring_whitespace(
        source,
        "ControlPin::WrN => (snapshot.pins.wr_n, true"
    ));
    assert!(source.contains("LOW ASSERTED"));
    assert!(source.contains("outer amber ring = signal ASSERTED"));
    assert!(source_contains_ignoring_whitespace(
        source,
        "label: \"INT\", kind: PinKind::Control(ControlPin::Interrupt)"
    ));
    assert!(source_contains_ignoring_whitespace(
        source,
        "label: \"PHI1\", kind: PinKind::Clock(ClockPin::Phi1"
    ));
    assert!(source_contains_ignoring_whitespace(
        source,
        "label: \"PHI2\", kind: PinKind::Clock(ClockPin::Phi2"
    ));
...
fn exact_undriven_cpu_data_pins_are_hi_z_not_front_panel_data() {
    ...
    assert!(
        source.contains("S-100 DI/DO and the front-panel DATA presentation are separate domains")
    );
}
...
fn cpu_control_pin_renderer_uses_backend_pin_truth_without_reconstructing_signals() {
    ...
    for expected in [
        ...
        assert!(
            source_contains_ignoring_whitespace(source, expected),
            "control pin must come from backend snapshot: {expected}"
        );
    }
...
fn package_and_teacher_keep_cpu_d_di_do_and_panel_data_separate() {
    ...
    for expected in [
        ...
        assert!(
            ...
            "package summary must expose split domain {expected}"
        );
        assert!(
            teacher.contains(expected),
            "Teacher must expose split domain {expected}"
        );
    }
    ...
    assert!(
        !diagram.contains("snapshot.data.map(|value| value & (1u8 << bit)"),
        "DIP-40 D pins must never consume the compatibility/front-panel DATA field"
    );
}
