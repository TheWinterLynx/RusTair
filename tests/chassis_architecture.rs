...
fn chassis_source_contains_no_cpu_or_deref_escape_hatch() {
    ...
    assert!(
        !source.contains("Cpu8080"),
        "the physical chassis must not own a Fast CPU"
    );
    assert!(
        !source.contains("Deref"),
        "the chassis migration must not hide ownership behind Deref"
    );
    assert!(
        !source.contains("DerefMut"),
        "the chassis migration must preserve explicit field ownership"
    );
    assert!(
        !source.contains("pub running: bool"),
        "RUN must not be a public chassis-side latch"
    );
}
