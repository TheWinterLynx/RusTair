...
fn independent_di_and_ei_targets_remain_part_of_the_card_configuration() {
    ...
    assert!(CONFIG_SOURCE.contains("pub const fn target(self,"));

    let config = compact(CONFIG_SOURCE);
    assert!(config.contains("0=>Some(self.port0)"));
    assert!(config.contains("1=>Some(self.port1)"));
    assert!(config.contains("_=>None"));
}
