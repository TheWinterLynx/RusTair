...
fn machine(compatibility: bool, output: bool) -> BackendHost {
    ...
    hardware
        .set_slot(
            2,
            Some(if compatibility {
                S100InstalledCardConfig::FastRamCompatibility(FastRamCompatibilityConfig::no_wait(
                    0, 32768,
                ))
            } else {
                S100InstalledCardConfig::Ram(S100RamCardConfig::fully_populated(
...
fn retained_output_has_identical_cpu_and_uart_results_in_full_and_observed_partial() {
    for compatibility in [false, true] {
        ...
        assert_eq!(
            full.serial_tx_complete(BackendSerialPort::Port1),
            partial.serial_tx_complete(BackendSerialPort::Port1)
        );
...
fn gui_batch_with_migrated_ram_and_pending_console_output() {
    for compatibility in [false, true] {
        for output in [false, true] {
            ...
            assert_eq!(
                machine.intel8080_state().total_t_states.unwrap() - before,
                1_000_000
            );
            ...
            eprintln!(
                "[GUI BATCH] compatibility={compatibility} retained_output={output}: {elapsed:.3?}, Full={:.2}% Partial={:.2}%",
                stats.full_percent(),
