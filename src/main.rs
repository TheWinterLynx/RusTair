...
impl RusTairApp {
    ...
    fn momentary(&mut self, ui:&mut egui::Ui, origin:Pos2, scale:f32, x:f32, y:f32, label:&str) -> Option<bool> {
        ...
        let tex=if resp.is_pointer_button_down_on(){
            if down{self.tex.switch_down.as_ref()}else{self.tex.switch_up.as_ref()}
        }else{self.tex.switch_centre.as_ref()};
    ...
    fn set_altair_power(&mut self, on: bool) {
        ...
        else { self.audio.stop_loop("altair-fan"); }
    }

    fn draw_altair(&mut self, ui:&mut egui::Ui) {
        let a=ui.available_size();
        let scale=(a.x/PANEL_W).min(a.y/PANEL_H).clamp(0.2,2.5);
        let (r,_)=ui.allocate_exact_size(Vec2::new(PANEL_W*scale,PANEL_H*scale),Sense::hover());
        let o=r.min;
    ...
    fn process_tty_io(&mut self, ctx:&egui::Context) {
        while let Some(b)=self.machine.bus.serial_tx.pop_front(){
            ...
            let events=self.tty.print_serial(b);
    ...
    fn draw_pressed_key(&self, ui:&mut egui::Ui, origin:Pos2, scale:f32) {
        ...
        let k=teletype::KEYS[index];
        let source=Rect::from_min_max(
            Pos2::new(k.x/TTY_W,k.y/TTY_H),
            Pos2::new((k.x+k.w)/TTY_W,(k.y+k.h+40.0)/TTY_H),
        );
        let target=Rect::from_min_size(
            origin+Vec2::new(k.x*scale,k.y*scale),
            Vec2::new(k.w*scale,(k.h+40.0)*scale),
        );
        if let Some(body)=&self.tex.tty_body { Self::image_uv(ui,body,target,source); }

        let key_source=Rect::from_min_max(
            Pos2::new(k.x/TTY_W,k.y/TTY_H),
            Pos2::new((k.x+k.w)/TTY_W,(k.y+k.h)/TTY_H),
        );
        let key_target=Rect::from_min_size(
            origin+Vec2::new(k.x*scale,(k.y+self.key_displacement)*scale),
            Vec2::new(k.w*scale,k.h*scale),
        );
    ...
    fn draw_teletype(&mut self, ui:&mut egui::Ui) {
        let a=ui.available_size();
        let scale=(a.x/TTY_W).min(a.y/TTY_H).clamp(0.12,1.5);
        let (r,resp)=ui.allocate_exact_size(Vec2::new(TTY_W*scale,TTY_H*scale),Sense::click_and_drag());
        let o=r.min;

        if let Some(t)=&self.tex.tty_body{Self::image(ui,t,r);}
        else{ui.painter().rect_filled(r,0.,Color32::from_rgb(80,76,65));}
        if let Some(t)=&self.tex.tty_keys{Self::image(ui,t,r);}

        let paper=Rect::from_min_size(
            o+Vec2::new(teletype::PRINT_LEFT*scale,teletype::PRINT_TOP*scale),
            Vec2::new(teletype::PRINTABLE_WIDTH*scale,TTY_H*0.27*scale),
        );
        let char_width=self.tty.char_width_image_px();
        let fs=(char_width*1.63*scale).max(5.0);
        let family=FontFamily::Name("teletype".into());
        ui.painter().text(
            paper.left_bottom(),
            egui::Align2::LEFT_BOTTOM,
            &self.tty.output,
            FontId::new(fs,family),
        ...
        if self.tty.mode!=TtyMode::Off {
            if let Some(t)=&self.tex.tty_head{
                ...
                let hr=Rect::from_min_size(
                    o+Vec2::new(x*scale,teletype::PRINT_HEAD_TOP*scale),
        ...
        let lls=Vec2::new(TTY_W*0.18*scale,288.*(TTY_W*0.18/349.)*scale);
        let ll=Rect::from_min_size(Pos2::new(r.right()-lls.x,r.bottom()-lls.y),lls);
        if let Some(t)=&self.tex.tty_line_local{Self::image(ui,t,ll);}

        if resp.drag_started() || (resp.is_pointer_button_down_on() && self.pressed_key.is_none()) {
            if let Some(p)=resp.interact_pointer_pos() {
                if ll.contains(p) {
                    let xp=(p.x-ll.left())/ll.width();
                    let yp=(p.y-ll.top())/ll.height();
                    if yp<0.52 { self.set_tty_mode(TtyMode::Off); }
                    else if xp<0.40 && yp>0.40 && yp<0.80 { self.set_tty_mode(TtyMode::Line); }
                    else if xp>0.56 && yp>0.40 && yp<0.80 { self.set_tty_mode(TtyMode::Local); }
                } else {
                    let ix=(p.x-r.left())/scale;
                    let iy=(p.y-r.top())/scale;
        ...
        if !pointer_down { self.release_tty_key(); }
        self.draw_pressed_key(ui,o,scale);

        if let Some(t)=&self.tex.tty_knob{
            let kw=TTY_W*0.06*scale;
            let kh=kw*117./116.;
            let base=r.right()-TTY_W*0.06*scale-kw;
            let sh=match self.tty.mode{TtyMode::Line=>-0.35*kw,TtyMode::Off=>0.,TtyMode::Local=>0.35*kw};
            let kr=Rect::from_min_size(Pos2::new(base+sh,r.bottom()-TTY_H*0.022*scale-kh),Vec2::new(kw,kh));
            Self::image(ui,t,kr);
        }
    }
    ...
    fn update_paper_tape(&mut self) {
        ...
        if self.machine.bus.serial_rx.is_empty() {
            if let Some(byte)=self.tty.next_tape_byte() { self.machine.bus.serial_rx.push_back(byte); }
        }
    }
    ...
    fn load_paper_tape(&mut self) {
        ...
        match std::fs::read(&path) {
            Ok(bytes)=>{self.tty.load_tape(&bytes);self.status=format!("Paper tape loaded: {} bytes",bytes.len());},
...
impl eframe::App for RusTairApp {
    fn update(&mut self,ctx:&egui::Context,_frame:&mut eframe::Frame){
        ...
        if let Some(until)=self.reset_flash_until{
            if now>=until{self.machine.address_leds=0;self.machine.bus.data_leds=0;self.reset_flash_until=None;}
        }
        if self.machine.running{
            let cycles=(CLOCK_HZ as f64*dt.as_secs_f64())as u32;
            self.machine.run_cycles(cycles.clamp(1,10000));
            ctx.request_repaint();
        }
        ...
        egui::TopBottomPanel::top("menu").show(ctx,|ui|egui::MenuBar::new().ui(ui,|ui|{
            ui.menu_button("File",|ui|{
                ...
                if ui.button("Load bundled Microsoft 4K BASIC").clicked(){
                    match std::fs::read("assets/4kbas32.bin"){
                        Ok(bytes)=>{self.machine.bus.load(0,&bytes);self.machine.cpu.pc=0;self.status="Loaded Microsoft 4K BASIC".into();},
            ...
            ui.separator();
            ui.selectable_value(&mut self.view,View::Altair,"ALTAIR 8800");
            ui.selectable_value(&mut self.view,View::Teletype,"ASR-33 TELETYPE");

            if self.view==View::Teletype{
                ui.separator();
                ...
                if ui.button(punch_label).clicked(){
                    if self.tty.capture_to_tape {self.tty.capture_to_tape=false;self.save_punched_tape();}
                    else {self.tty.tape_out.clear();self.tty.capture_to_tape=true;}
                }
            }
        ...
        egui::CentralPanel::default().show(ctx,|ui|ui.centered_and_justified(|ui|match self.view{
            View::Altair=>self.draw_altair(ui),
            View::Teletype=>self.draw_teletype(ui),
        }));
