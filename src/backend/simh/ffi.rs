...
impl FrontPanelApi {
    ...
    pub fn load(path: &Path) -> Result<Self, FrontPanelLoadError> {
        ...
        macro_rules! symbol {
            ($name:literal, $ty:ty) => {{
                ...
                if ptr.is_null() {
                    ...
                    return Err(FrontPanelLoadError::MissingSymbol {
                        symbol: $name,
                        code,
                    });
                }
        ...
        Ok(Self {
            ...
            start_simulator_debug: symbol!(
                "sim_panel_start_simulator_debug",
                StartSimulatorDebugFn
            ),
    ...
    pub unsafe fn start_simulator_debug(
        &self,
        sim_path: *const c_char,
        sim_config: *const c_char,
        device_panel_count: usize,
        ...
        unsafe {
            (self.start_simulator_debug)(sim_path, sim_config, device_panel_count, debug_file)
        }
    }

    pub unsafe fn destroy(&self, panel: *mut PANEL) -> c_int {
        unsafe { (self.destroy)(panel) }
    }

    pub unsafe fn exec_halt(&self, panel: *mut PANEL) -> c_int {
        unsafe { (self.exec_halt)(panel) }
    }

    pub unsafe fn exec_boot(&self, panel: *mut PANEL, device: *const c_char) -> c_int {
        unsafe { (self.exec_boot)(panel, device) }
    }

    pub unsafe fn exec_start(&self, panel: *mut PANEL) -> c_int {
        unsafe { (self.exec_start)(panel) }
    }

    pub unsafe fn exec_run(&self, panel: *mut PANEL) -> c_int {
        unsafe { (self.exec_run)(panel) }
    }

    pub unsafe fn exec_step(&self, panel: *mut PANEL) -> c_int {
        unsafe { (self.exec_step)(panel) }
    }

    pub unsafe fn halt_text(&self, panel: *mut PANEL) -> *const c_char {
        unsafe { (self.halt_text)(panel) }
    }

    pub unsafe fn gen_examine(
        &self,
        panel: *mut PANEL,
    ...
    pub unsafe fn device_debug_mode(
        &self,
        panel: *mut PANEL,
        device: *const c_char,
        set_unset: c_int,
    ...
    pub unsafe fn dismount(&self, panel: *mut PANEL, device: *const c_char) -> c_int {
        unsafe { (self.dismount)(panel, device) }
    }

    pub unsafe fn get_state(&self, panel: *mut PANEL) -> OperationalState {
        unsafe { (self.get_state)(panel) }
    }

    pub unsafe fn get_error(&self) -> *const c_char {
