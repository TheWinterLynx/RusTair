...
/// Open-SIMH-backed implementation of the classic `ALTAIR` simulator.
///
/// This first backend deliberately exposes only behavior the upstream simulator
/// and FrontPanel API can represent reliably. In particular, the classic SIMH
/// target does not expose exact S-100 activity, Intel T-state totals, or a CPU
/// HLT latch distinct from the simulator's operational Halt state.
...
impl SimhAltairBackend {
    ...
    fn set_pc(&mut self, pc: u16) -> BackendResult<()> {
        self.session_mut()?
            .deposit_register_u32(super::altair_registers::PC, u32::from(pc))
...
impl MachineBackend for SimhAltairBackend {
    ...
    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            ...
            // The classic target routes its first 2SIO channel through the SIMH
            // console and its second through PTR/PTP; RusTair endpoint routing is
            // not connected to those paths yet.
    ...
    fn execution_model(&self) -> BackendExecutionModel {
        BackendExecutionModel::ExternalProcess
    }
    ...
    fn cpu_state(&mut self) -> BackendResult<CpuState> {
        ...
        Ok(CpuState {
            a: registers.a,
            b: (registers.bc >> 8) as u8,
            c: registers.bc as u8,
            d: (registers.de >> 8) as u8,
            e: registers.de as u8,
            h: (registers.hl >> 8) as u8,
            l: registers.hl as u8,
            flags: registers.flags_8080(),
            pc: registers.pc,
            sp: registers.sp,
            inte: registers.inte,
            halted: None,
    ...
    fn front_panel_state(&mut self) -> BackendResult<FrontPanelState> {
        ...
        Ok(FrontPanelState {
            ...
            running,
            switches: registers.switch_register,
            // This is a coarse programmer-visible address/data view, not exact
            // S-100 bus activity. `exact_bus_activity=false` advertises that.
            address,
            data,
    ...
    fn run(&mut self) -> BackendResult<()> {
        self.session_mut()?
            .run()
            .map_err(|error| backend_error("SIMH RUN", error))
    }

    fn halt(&mut self) -> BackendResult<()> {
        self.session_mut()?
            .halt()
            .map_err(|error| backend_error("SIMH HALT", error))?;
        self.refresh_stopped_panel_latch()
    }
    ...
    fn step(&mut self) -> BackendResult<()> {
        ...
        self.session_mut()?
            .step()
            .map_err(|error| backend_error("SIMH STEP", error))?;
        self.refresh_stopped_panel_latch()
    }
    ...
    fn release_reset(&mut self) -> BackendResult<()> {
        self.unsupported("physical RESET release")
    }
    ...
    fn panel_examine(&mut self, next: bool) -> BackendResult<()> {
        ...
        let address = if next {
            ...
            self.registers()?.switch_register
        };
        let data = self
            .session()?
            .read_byte(address)
            .map_err(|error| backend_error("SIMH EXAMINE", error))?;
        self.set_pc(address)?;
    ...
    fn panel_deposit(&mut self, next: bool) -> BackendResult<()> {
        ...
        self.session_mut()?
            ...
            .map_err(|error| backend_error("SIMH DEPOSIT", error))?;
        if next {
            self.set_pc(address)?;
        }
        self.panel_address_latch = address;
    ...
    fn switch_register(&mut self) -> BackendResult<u16> {
        Ok(self.registers()?.switch_register)
    }
    ...
    fn configure_serial_board(&mut self, board: SerialBoard) -> BackendResult<()> {
        if board == SerialBoard::TwoSio88 {
            ...
            self.unsupported("MITS 88-SIO; classic SIMH ALTAIR is fixed to 88-2SIO")
        }
    }
    ...
    fn serial_receive(&mut self, _port: BackendSerialPort, _byte: u8) -> BackendResult<()> {
        self.unsupported("RusTair serial endpoint routing to SIMH console/PTR")
    }
    ...
    fn peek_memory(&mut self, address: u16) -> BackendResult<Option<u8>> {
        self.session()?
            .read_byte(address)
            .map(Some)
            .map_err(|error| backend_error("SIMH memory examine", error))
    }
    ...
    fn write_memory(
        ...
        self.session_mut()?
            .write_byte(address, value)
            .map_err(|error| backend_error("SIMH memory deposit", error))?;
        Ok(true)
    }
    ...
    fn load_bytes(&mut self, address: u16, bytes: &[u8]) -> BackendResult<()> {
        self.session_mut()?
            ...
            .map_err(|error| backend_error("SIMH memory load", error))
    }
}
