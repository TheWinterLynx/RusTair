...
mod tests {
    ...
    #[test]
    fn cycle_pint_enters_real_inta_and_latches_sinta_on_following_phi1() {
        let mut backend = CycleAccurateMachineBackend::default();
        backend.power(true).unwrap();
        backend.assert_reset().unwrap();
        backend.release_reset().unwrap();
        ...
        let t1 = backend.teaching_snapshot().expect("INTA T1 sample");
        assert_eq!(t1.machine_cycle, MachineCycle::InterruptAck.into());
        ...
        backend.service_execution(1).unwrap();
        let t2 = backend.teaching_snapshot().expect("INTA T2 sample");
        assert_eq!(t2.t_state, TState::T2.into());
        assert_eq!(t2.status_word, Some(0x23));
        assert_eq!(t2.status.int_ack, Some(true));
        assert!(!backend.machine().bus.raw_s100_psync());
        assert!(backend.machine().bus.raw_s100_pdbin());
    ...
    #[test]
    fn cycle_pint_wakes_halt_and_latches_2b_status_on_following_phi1() {
        let mut backend = CycleAccurateMachineBackend::default();
        backend.power(true).unwrap();
        backend.assert_reset().unwrap();
        backend.release_reset().unwrap();
        ...
        let t1 = backend.teaching_snapshot().expect("halt INTA T1");
        assert_eq!(t1.machine_cycle, MachineCycle::InterruptAckWhileHalt.into());
        assert_eq!(t1.t_state, TState::T1.into());
        assert_eq!(t1.cpu_data, Some(0x2b));
        assert_eq!(t1.s100_do, Some(0x2b));
        assert_eq!(t1.pins.inte, Some(false));
        assert!(!backend.cpu().is_halted());
        assert!(!backend.cpu().interrupts_enabled());

        backend.service_execution(1).unwrap();
        let t2 = backend.teaching_snapshot().expect("halt INTA T2");
        assert_eq!(t2.status_word, Some(0x2b));
        assert_eq!(t2.status.int_ack, Some(true));
