...
/// therefore part of execution, not a viewer-only reconstruction. The resulting
/// PHI2 sample is converted by the CPU-board adapter into the canonical S-100
/// state consumed by the front panel and hardware cards.
...
impl CycleAccurateMachineBackend {
    ...
    fn tick_once_with_front_panel_data(
        ...
        // PINT is generated by the selected S-100 device and must be sampled at
        // the real processor boundary. Refresh the canonical backplane line
        // before both the T3 data source and the clock-edge input sample.
        ...
        let lines = self.machine.bus.cpu_control_lines();
        let trace = self.cpu.tick_with_pin_edges(
            Cpu8080Inputs {
                data_in,
        ...
        // T2 -> first TW transition before host execution freezes.
    ...
    fn reset_cycle_core_from_s100(&mut self) {
        ...
        self.machine.bus.refresh_interrupt_request_line();
        let lines = self.machine.bus.cpu_control_lines();
        let _ = self.cpu.tick_with_pin_edges(
            Cpu8080Inputs {
                ready: lines.ready,
                interrupt: lines.interrupt,
                hold: lines.hold,
