...
use crate::trace8080::{
    CpuSnapshot8080, InstructionEffect8080, InstructionTraceBuffer, InstructionTraceMetadata,
    collect_post_instruction_effects, collect_pre_instruction_effects,
};
...
use super::{
    ...
    BusTeachingAccuracy, BusTeachingSnapshot, CpuState, CycleAccurateMachineBackend,
    DebugStopReason, EmulationEngine, FrontPanelState, InstructionTraceSnapshot, IoPortActivity,
    IoTraceSnapshot, MachineBackend, MemoryWatchAccess, SerialModemLines, SioLogicalLines,
};
...
impl CycleHostBackend {
    ...
    fn finalize_pending_trace(
        ...
        p.effects.extend(collect_post_instruction_effects(
            p.bytes, p.before, after, &p.effects,
        ));
        let elapsed = inner
            .cpu()
            .total_t_states()
            .saturating_sub(p.start_t_states) as u32;
        ...
        let watch_stop = debug_control
            .stop_after_effects(p.address, &p.effects)
            .is_some();
        if instruction_trace.enabled() {
            instruction_trace
                .push_with_effects(p.address, p.bytes, p.before, after, delta, p.effects);
        }
        watch_stop
    }
    ...
    fn finish_instruction_trace_if_complete(&mut self) -> bool {
        ...
        if !completed && !self.inner.cpu().is_halted() {
            return false;
        }
    ...
    fn control_teaching_snapshot(&self) -> BusTeachingSnapshot {
        ...
        BusTeachingSnapshot {
            ...
            total_t_states: if powered {
                Some(self.inner.cpu().total_t_states())
            } else {
                None
            },
    ...
    fn debugger_step_one_machine_cycle(&mut self) -> BackendResult<()> {
        ...
        for _ in 0..32 {
            ...
            if self.inner.cpu().machine_cycle() != start_cycle
                ...
                || (self.inner.cpu().t_state() == TState::T1
                    && self.inner.cpu().total_t_states() > start_t)
            {
                break;
            }
        }
    ...
    fn debugger_step_one_instruction(&mut self) -> BackendResult<()> {
        ...
        for _ in 0..128 {
            self.inner.debugger_step_t_state_exact()?;
            if self.inner.cpu().completed_instructions() > start
                || self.inner.cpu().is_halted()
                || self.inner.cpu().is_holding()
            {
                break;
...
impl MachineBackend for CycleHostBackend {
    ...
    fn configure_memory_board_profile(&mut self, profile: RamBoardProfile) -> BackendResult<()> {
        ...
        self.inner
            .machine_mut()
            .configure_memory_board_profile(profile);
        if powered {
            self.inner.assert_reset()?;
            self.inner.release_reset()?;
            self.teaching_reset_seen = true;
    ...
    fn protect_current_board(&mut self, p: bool) -> BackendResult<()> {
        self.inner.protect_current_board(p)
    }
    fn switch_register(&mut self) -> BackendResult<u16> {
        self.inner.switch_register()
    }
    fn set_switch_register(&mut self, v: u16) -> BackendResult<()> {
    ...
    fn serial_modem_lines(
        ...
        Ok(self
            ...
            .machine()
            .bus
            .serial_modem_lines(p.index())
            .map(SerialModemLines::from))
    }
    ...
    fn installed_ram_bytes(&mut self) -> BackendResult<usize> {
        Ok(self.inner.machine().installed_ram_bytes())
    }
    fn peek_memory(&mut self, a: u16) -> BackendResult<Option<u8>> {
        self.inner.peek_memory(a)
    }
    fn write_memory(&mut self, a: u16, v: u8, r: bool) -> BackendResult<bool> {
        self.invalidate_partial_trace_for_external_memory_change();
        self.inner.write_memory(a, v, r)
    }
    fn load_bytes(&mut self, a: u16, b: &[u8]) -> BackendResult<()> {
        self.inner.load_bytes(a, b)?;
        self.reset_debugger_epoch();
        Ok(())
    }
    fn memory_is_protected(&mut self, a: u16) -> BackendResult<bool> {
        Ok(self.inner.machine().bus.is_protected(a))
    }
    ...
    fn arm_basic32_full_memory_probe_guard(&mut self) -> BackendResult<bool> {
        ...
        Ok(self
            .inner
            .machine_mut()
            .arm_basic32_full_memory_probe_guard())
    }
    fn begin_cpu_diagnostic_meter(
        ...
        self.inner
            .machine_mut()
            .begin_cpu_diagnostic_meter(n, s, l, ei, et);
        Ok(())
    }
    ...
    fn io_port_activity(&mut self, p: u8) -> BackendResult<IoPortActivity> {
        Ok(self.inner.machine().bus.io_port_activity(p))
    }
    ...
    fn set_io_trace_enabled(&mut self, e: bool) -> BackendResult<()> {
        self.inner.machine_mut().bus.set_io_trace_enabled(e);
        Ok(())
    }
    ...
    fn set_instruction_trace_enabled(&mut self, e: bool) -> BackendResult<()> {
        self.instruction_trace.set_enabled(e);
        if !e {
            self.clear_pending_instruction_trace();
        }
        Ok(())
    ...
    fn bus_teaching_snapshot(&mut self) -> BackendResult<Option<BusTeachingSnapshot>> {
        ...
        s.current_chassis = Some(BusChassisSnapshot::from_altair_machine(
    ...
    fn debugger_breakpoints(&mut self) -> BackendResult<Vec<u16>> {
        Ok(self.debug_control.breakpoints())
    }
    fn debugger_set_breakpoint(&mut self, a: u16, e: bool) -> BackendResult<()> {
        self.debug_control.set_breakpoint(a, e);
        Ok(())
    }
    ...
    fn debugger_set_watchpoint(
        &mut self,
        a: u16,
        x: Option<MemoryWatchAccess>,
    ) -> BackendResult<()> {
        self.debug_control.set_watchpoint(a, x);
        Ok(())
    }
    ...
    fn debugger_run_to(&mut self, a: u16) -> BackendResult<()> {
        self.debug_control.set_run_to(a);
        self.run()
    }
    fn debugger_run_to_with_sp(&mut self, a: u16, sp: u16) -> BackendResult<()> {
        self.debug_control.set_run_to_with_sp(a, sp);
        self.run()
    }
...
mod tests {
    ...
    fn bus_teacher_exposes_power_and_reset_without_fake_t_states() {
        ...
        assert_eq!(
            on.status.inte,
            Some(backend.inner.cpu().interrupts_enabled())
        );
    ...
    fn lifecycle_teacher_raw_status_and_data_are_not_derived_from_optical_lamps() {
        ...
        backend
            ...
            .machine_mut()
            .bus
            .debug_set_panel_lamp_snapshot_for_test(crate::machine::PanelLampSnapshot::default());
    ...
    fn wrapper_dispatches_cycle_step_without_exposing_chassis_to_app() {
        let mut backend = CycleHostBackend::default();
        backend
            .configure_memory(RamSize::K1, RamInit::Zeroed)
            .unwrap();
        backend.power(true).unwrap();
        backend.assert_reset().unwrap();
        backend.release_reset().unwrap();
