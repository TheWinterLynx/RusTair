...
use super::{
    ...
    EmulationEngine, FrontPanelState, MachineBackend,
};
...
/// migration escape hatch while existing app code is moved behind the contract.
...
mod tests {
    ...
    fn cpu_snapshot_is_backend_neutral() {
        ...
        assert_eq!(state.halted, Some(false));
        assert_eq!(state.total_t_states, Some(1234));
    }

    #[test]
    fn fast_backend_identifies_timing_limits() {
    ...
    fn switch_register_adapter_sets_exact_value() {
        let mut backend = NativeMachineBackend::default();
        backend.set_switch_register(0xa55a).unwrap();
        assert_eq!(backend.switch_register().unwrap(), 0xa55a);
        backend.set_switch_register(0x0f0f).unwrap();
        assert_eq!(backend.switch_register().unwrap(), 0x0f0f);
    }
    ...
    fn serial_contract_routes_both_88_2sio_channels() {
        ...
        backend.serial_receive(BackendSerialPort::Port0, b'A').unwrap();
        backend.serial_receive(BackendSerialPort::Port1, b'B').unwrap();
        assert_eq!(backend.serial_rx_len(BackendSerialPort::Port0).unwrap(), 1);
        assert_eq!(backend.serial_rx_len(BackendSerialPort::Port1).unwrap(), 1);
    }
