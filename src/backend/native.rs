...
impl MachineBackend for NativeMachineBackend {
    ...
    fn debugger_input_port(&mut self, port: u8) -> BackendResult<u8> {
        // A debugger read may intentionally execute the register's side effects,
        // but it is not an 8080 instruction. Discard any stale Fast wait debt
        // before the observation and the PRDY/Tw wait generated by this read
        // afterwards so the next guest instruction owns only its own T-states.
        let _ = crate::cpu8080::Bus::take_wait_states(&mut self.machine.bus);
        let value = self.machine.bus.debugger_input_port(port);
        let _ = crate::cpu8080::Bus::take_wait_states(&mut self.machine.bus);
        Ok(value)
    }
