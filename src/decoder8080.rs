...
pub fn decode_8080(opcode: u8, b1: u8, b2: u8) -> DecodedInstruction {
    ...
    match opcode {
        ...
        0xc9 | 0xd9 => decoded(
            ...
            MemoryAccess::StackRead,
            IoAccess::None,
            ControlFlow::Return { condition: None },
            opcode == 0xd9,
        ),
        0xcd | 0xdd | 0xed | 0xfd => decoded(
            opcode,
            "CALL",
            vec![format!("${value:04X}")],
            3,
...
mod tests {
    ...
    fn undocumented_8080_aliases_are_explicit() {
        for opcode in [
            ...
            assert!(
                decode_8080(opcode, 0x34, 0x12).undocumented_alias,
                "opcode {opcode:02X}"
            );
        }
