...
impl RusTairApp {
    ...
    fn draw_io_header(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            ui.separator();
            ui.label(self.physical_serial_inventory_label());
            ui.separator();
            ui.monospace(format!("PC {:04X}h", cpu.pc));
            ui.separator();
            ui.label(if panel.running { "RUNNING" } else { "STOPPED" });
            ui.separator();
    ...
    fn draw_status_interpretation(&self, ui: &mut egui::Ui, port: u8, value: u8) {
        ...
        match binding.kind {
            PhysicalSerialPortKind::SioStatus => {
                ui.strong(format!("Slot {} · 88-SIO status", binding.slot));
                ui.monospace(format!("{:08b}", value));
                ui.label(format!(
                    "bit 0 = {} → RX {}",
                    value & 0x01,
            ...
            PhysicalSerialPortKind::TwoSioStatus(channel) => {
                ui.strong(format!(
                    "Slot {} · 88-2SIO Port {channel} status",
                    binding.slot
                ));
                ui.monospace(format!("{:08b}", value));
                ui.label(format!(
            ...
            super::collapsible_section(ui, "Serial DATA-port tools", true, |ui| {
                    ...
                    if ui.small_button("Inject RX").clicked() {
                        match Self::parse_hex_sequence(&inject_text) {
                            Ok(bytes) => {
                                ...
                                for byte in bytes {
                                    if self.machine.debugger_inject_serial_rx(selected, byte) {
                                        injected += 1;
                                    }
                                }
                                self.status = format!(
                ...
                ui.horizontal_wrapped(|ui| {
                    if ui.small_button("Clear RX").clicked() {
                        self.machine.debugger_clear_serial_rx(selected);
                    }
                    ...
                    if ui.small_button("Clear TX").clicked() {
                        self.machine.debugger_clear_serial_tx(selected);
                    }
                });
            });
        }
    ...
    fn draw_com_trace(&mut self, ui: &mut egui::Ui, generation: u64) {
        ui.horizontal_wrapped(|ui| {
            ...
            ui.label(format!(
                "Mode: {}",
                self.external_com.config.character_mode.label()
            ));
            ui.separator();
    ...
    fn draw_io_help(&self, ui: &mut egui::Ui) {
        ...
        ui.label(
            "5. Emulated I/O shows UART enqueue and the DATA-port value returned to the guest.",
        );
        ui.label("6. STATUS polling is hidden by default so busy-wait loops do not bury useful DATA events.");
        ui.label(
            "7. Inject RX bypasses host transports for an A/B test of UART and guest software.",
        );
    }
    ...
    fn draw_io_sidebar(&mut self, ui: &mut egui::Ui, selected: &mut u8) {
        egui::ScrollArea::vertical()
            ...
            .auto_shrink([false, false])
            .show(ui, |ui| {
                super::collapsible_section(ui, "Selected I/O port", true, |ui| {
                    self.draw_selected_port(ui, *selected)
                });
                ui.separator();
                super::collapsible_section(ui, "I/O port map 00h–FFh", false, |ui| {
                    self.draw_port_map(ui, selected)
                });
                ui.separator();
                super::collapsible_section(ui, "How to use the serial traces", false, |ui| {
                    self.draw_io_help(ui)
                });
            });
    }
