...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::app) struct ResolvedDiagnosticPort {
    pub(in crate::app) board: SerialBoard,
    pub(in crate::app) slot: usize,
    pub(in crate::app) channel: u8,
    pub(in crate::app) status_port: u8,
    pub(in crate::app) data_port: u8,
}

impl ResolvedDiagnosticPort {
    pub(in crate::app) fn label(self) -> String {
        match self.board {
            SerialBoard::Sio88 => format!(
                "Slot {} · 88-SIO Port 0 [{:02X}h/{:02X}h]",
                self.slot, self.status_port, self.data_port
            ),
            SerialBoard::TwoSio88 => format!(
                "Slot {} · 88-2SIO Port {} [{:02X}h/{:02X}h]",
                self.slot, self.channel, self.status_port, self.data_port
            ),
        }
    }
}
...
impl DiagnosticSerialPort {
    pub(in crate::app) const fn connection(self) -> SerialConnection {
        match self {
    ...
    ) -> Option<ResolvedDiagnosticPort> {
        let (slot, card) = hardware.active_serial_card_slot()?;
        match (card, self) {
            (S100InstalledCardConfig::Mits88Sio(config), Self::Port0) => {
                Some(ResolvedDiagnosticPort {
            ...
            (S100InstalledCardConfig::Mits88Sio(_), Self::Port1) => None,
            (S100InstalledCardConfig::Mits88TwoSio { straps, .. }, Self::Port0) => {
                Some(ResolvedDiagnosticPort {
            ...
            (S100InstalledCardConfig::Mits88TwoSio { straps, .. }, Self::Port1) => {
                Some(ResolvedDiagnosticPort {
                    board: SerialBoard::TwoSio88,
                    slot,
                    channel: 1,
                    status_port: straps.address.port1_status(),
                    data_port: straps.address.port1_data(),
                })
            }
            _ => None,
        }
...
fn build_cpm_diagnostic_environment(
    endpoint: ResolvedDiagnosticPort,
    bdos_base: u16,
...
impl RusTairApp {
    pub(in crate::app) fn start_cpu_diagnostic_dialog(&mut self, port: DiagnosticSerialPort) {
        ...
        if port.resolve(hardware).is_none() {
            let reason = if hardware.active_serial_board().is_none() {
                "no MITS 88-SIO/88-2SIO is installed"
            } else {
                "the installed MITS 88-SIO provides only Port 0"
            };
            self.report_load_error(format!(
                "CPU diagnostic cannot use Port {} because {reason}. Configure the physical serial card under S-100 Chassis / Cards first.",
                match port {
                    DiagnosticSerialPort::Port0 => 0,
                    DiagnosticSerialPort::Port1 => 1,
                }
            ));
            return;
        }
    ...
    fn poll_cpu_diagnostic_result(&mut self, ctx: &egui::Context) {
        ...
        let speed_label = ctx
            .data(|data| data.get_temp::<String>(speed_id))
            .unwrap_or_else(|| {
                emulation_speed_label(
                    self.config.preferences.emulation_speed,
                    configured_cpu_board(self),
        ...
        egui::Window::new("CPU diagnostic complete")
            ...
            .show(ctx, |ui| {
                ...
                match reference_match {
                    Some(true) => {
                        ui.strong(
                            "REFERENCE MATCH — instruction count and T-state total are exact.",
                        );
                    }
                    Some(false) => {
                    ...
                    None => {
                        ui.strong(
                            "Measurement complete — no reference totals are registered for this file.",
                        );
                    }
                }
                ui.add_space(8.0);
    ...
    fn load_cpu_diagnostic(
        ...
        let hardware = self.config.machine.s100_hardware;
        let Some(endpoint) = port.resolve(hardware) else {
            self.report_load_error(format!(
                "CPU diagnostic {} cannot start because the requested serial port is not present on the installed S-100 serial hardware.",
                path.display()
            ));
            return;
        };
        ...
        let environment = build_cpm_diagnostic_environment(endpoint, bdos_base);
...
mod tests {
    use super::*;
    use crate::config::{SioAddressPair, TwoSioAddressBlock, TwoSioInterruptWiring, TwoSioStraps};
    ...
    fn cpm_vector_exposes_high_bdos_address_for_8080exm() {
        let env = build_cpm_diagnostic_environment(
            endpoint(SerialBoard::TwoSio88, 0, 0x10, 0x11),
            0xff00,
        );
    ...
    fn putc_uses_2sio_ready_semantics_on_both_ports() {
        ...
        let p1 = build_cpm_diagnostic_environment(
            endpoint(SerialBoard::TwoSio88, 1, 0x12, 0x13),
            0x7f00,
        );
        assert_eq!(
            &p0.bdos[0x2b..0x37],
    ...
    fn readdressed_serial_card_changes_only_mini_bdos_io_operands() {
        ...
        let two_sio = build_cpm_diagnostic_environment(
            endpoint(SerialBoard::TwoSio88, 1, 0x46, 0x47),
            0x7f00,
        );
        assert_eq!(two_sio.bdos[0x2d], 0x46);
        assert_eq!(two_sio.bdos[0x35], 0x47);
    }

    #[test]
    fn diagnostic_port_resolution_comes_from_installed_card_and_straps() {
        ...
        hardware
            ...
            .unwrap();
        let resolved = DiagnosticSerialPort::Port1.resolve(hardware).unwrap();
        assert_eq!(resolved.slot, 5);
    ...
    #[test]
    fn sio_port1_is_rejected_even_when_sio_is_readdressed() {
        let mut hardware = S100HardwareConfig::empty(S100ChassisConfig::altair_8800b(6)).unwrap();
        hardware
            .set_slot(1, Some(S100InstalledCardConfig::Mits8080Cpu))
            .unwrap();
        let mut sio = crate::config::SioHardwareConfig::default();
        sio.address = SioAddressPair::try_new(0x06).unwrap();
        hardware
            .set_slot(5, Some(S100InstalledCardConfig::Mits88Sio(sio)))
            .unwrap();
        assert!(DiagnosticSerialPort::Port1.resolve(hardware).is_none());
    }
}
