...
impl SuiteReport {
    fn passed(&self) -> bool {
        self.control.passed() && self.results.iter().all(reference_match)
    }
}
...
impl Bus for ProbeBus {
    ...
    fn set_inte(&mut self, enabled: bool) { self.inte_events.push(enabled); }
...
fn run_control_line_baseline() -> ControlLineReport {
    ...
    // EI must not enable INTE until the instruction following EI has completed.
    let mut bus = ProbeBus::default();
    bus.mem[0..3].copy_from_slice(&[0xfb, 0x00, 0xf3]); // EI; NOP; DI
    let mut cpu = Cpu8080::new();
    let ei_t = cpu.step(&mut bus);
    let ei_delayed = !cpu.inte && ei_t == 4;
    let nop_t = cpu.step(&mut bus);
    let enabled_after_next = cpu.inte && nop_t == 4;
    let di_t = cpu.step(&mut bus);
    let di_disabled = !cpu.inte && di_t == 4;
    checks.push(ControlCheck {
        name: "EI delay / DI",
        passed: ei_delayed && enabled_after_next && di_disabled,
        detail: format!(
            "EI immediate INTE={} · after following NOP INTE={} · after DI INTE={}",
            !ei_delayed, enabled_after_next, cpu.inte
        ),
    });

    // HLT must acknowledge HALT, and an accepted interrupt must wake the CPU,
    // clear INTE and perform a normal RST stack push through interrupt acknowledge.
    let mut bus = ProbeBus::default();
    bus.mem[0] = 0x76;
    let mut cpu = Cpu8080::new();
    cpu.sp = 0x2000;
    ...
    // IN and OUT must use the CPU bus contract without knowing the attached device.
    let mut bus = ProbeBus::default();
    ...
    // The present instruction-level machine cannot insert true TW states yet,
    // but its front-panel READY/WAIT gating and HOLD/HLDA arbitration are part
    // of the baseline that the cycle-accurate core must preserve and refine.
...
impl RusTairApp {
    pub(in crate::app) fn effective_emulation_speed(&self) -> EmulationSpeed {
        if self.embedded_diagnostics.active_test.is_some()
            || self.embedded_diagnostics.external_active
    ...
    pub(in crate::app) fn draw_cpu_diagnostics_menu(&mut self, ui: &mut egui::Ui) {
        ...
        ui.small(
            "Embedded Intel 8080 diagnostics run as real guest code at 0100h through the selected MITS serial board. The suite also includes a RusTair control-line baseline for interrupt, HALT, I/O and bus-arbitration behaviour.",
        );
        ui.separator();

        ui.menu_button("Test speed", |ui| {
            for speed in DiagnosticRunSpeed::ALL {
                let selected = self.embedded_diagnostics.speed == speed;
                if ui
                    .add_enabled(!running, egui::Button::new(speed.label()).selected(selected))
                    .clicked()
                {
        ...
        ui.small(format!("Selected: {}", self.embedded_diagnostics.speed.label()));
        ...
        ui.small("Full suite: RusTair control-line baseline + 8080PRE + TST8080 + CPUTEST + 8080EXM. Requires at least 32 KiB RAM.");
        ...
        if ui
            ...
            .clicked()
        {
            self.embedded_diagnostics.external_active = true;
            self.start_cpu_diagnostic_dialog(self.embedded_diagnostics.port);
            ui.close();
        }
        ui.small("External .COM uses the same selected test speed and serial output while it is running.");
        ...
        if picker_open {
            ui.small("A Windows diagnostic picker is open; guest execution is paused until it closes.");
        }
    }

    pub(in crate::app) fn poll_embedded_cpu_diagnostics(&mut self, ctx: &egui::Context) {
        // The native external-file picker is owned by cpu_diagnostics.rs. If it
        // was cancelled or failed before a guest was started, release the speed
        // override on the following frame.
        if self.embedded_diagnostics.external_active && self.diagnostic_file_dialog.is_none() {
            let cancelled = self.status.contains("selection cancelled")
                || self.status.starts_with("LOAD ERROR:")
                || self.status.starts_with("Load failed:");
            if cancelled {
                self.embedded_diagnostics.external_active = false;
            }
        }

        let should_take_result = self.embedded_diagnostics.active_test.is_some()
            || self.embedded_diagnostics.external_active
            || self.embedded_diagnostics.suite.is_some();
        if should_take_result {
    ...
    fn start_embedded_cpu_suite(&mut self) {
        ...
        if self.config.machine.serial_board == SerialBoard::Sio88
            && self.embedded_diagnostics.port == cpu_diagnostics::DiagnosticSerialPort::Port1
        {
            self.report_load_error("The installed MITS 88-SIO has no Port 1. Select Port 0 or install the MITS 88-2SIO.");
            return;
        }
    ...
    fn load_embedded_classic_test(
        ...
        let Some(environment) = build_cpm_environment(board, port, bdos_base) else {
            self.report_load_error(format!("{} is unavailable on {}.", port_label(board, port), board.label()));
            return false;
        };

        if !self.machine.powered { self.machine.power(true); }
        ...
        let endpoint = self.serial_router.device_on(connection)
            .map(Self::serial_device_name)
            .unwrap_or("no endpoint connected");
        self.status = format!(
            "Embedded CPU diagnostic running: {} — {} — {} — output via {} → {}",
            test.filename(), self.embedded_diagnostics.speed.label(),
            self.config.machine.ram_size.label(), port_label(board, port), endpoint
        );
        true
    }
    ...
    fn handle_embedded_cpu_result(&mut self, result: CpuDiagnosticResult) {
        ...
        if let Some(mut suite) = self.embedded_diagnostics.suite.take() {
            ...
            if suite.next_index < ClassicDiagnostic::SUITE.len() {
                let test = ClassicDiagnostic::SUITE[suite.next_index];
                suite.next_index += 1;
                self.embedded_diagnostics.suite = Some(suite);
                if !self.start_embedded_classic_test(test, true) {
                    self.embedded_diagnostics.suite = None;
                }
                return;
            }
    ...
    fn draw_embedded_individual_result(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("CPU diagnostic complete")
            ...
            .show(ctx, |ui| {
                ...
                ui.add_space(6.0);
                if matched { ui.strong("REFERENCE MATCH"); } else { ui.strong("REFERENCE MISMATCH"); }
                ui.add_space(8.0);
                egui::Grid::new("embedded-cpu-result-grid").num_columns(4).spacing([18.0, 5.0]).show(ui, |ui| {
                    ...
                    if let Some(expected) = result.expected_instructions {
                        ui.monospace(format_count(expected)); ui.monospace(format_diff(result.instructions, expected));
                    ...
                    if let Some(expected) = result.expected_t_states {
                        ui.monospace(format_count(expected)); ui.monospace(format_diff(result.t_states, expected));
                    } else { ui.label("—"); ui.label("—"); }
                    ui.end_row();
                });
                ui.add_space(8.0);
    ...
    fn draw_control_line_report(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("RusTair 8080 control-line baseline")
            ...
            .default_width(680.0)
            .show(ctx, |ui| {
                if passed { ui.strong("PASS — all baseline checks succeeded."); }
                else { ui.strong("FAIL — at least one baseline check failed."); }
                ui.add_space(8.0);
                ...
                ui.add_space(8.0);
                ui.small("READY/WAIT here freezes the current instruction-level front-panel gating contract. True T-state TW insertion will be validated by the future cycle-accurate core.");
                if ui.button("OK").clicked() { dismiss = true; }
            });
    ...
    fn draw_suite_report(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("CPU diagnostic suite complete")
            ...
            .default_width(760.0)
            .show(ctx, |ui| {
                if passed { ui.heading("ALL TESTS PASS"); ui.strong("All classic references match exactly."); }
                else { ui.heading("SUITE FAILURE"); ui.strong("Inspect the failing row(s) below."); }
                ui.add_space(8.0);
