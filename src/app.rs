...
impl RusTairApp {
    ...
    /// Returns Some(true) for the lower/down position and Some(false) for the
    /// upper position. All function switches spring back to centre on release.
    fn momentary(
        &mut self,
        ui: &mut egui::Ui,
        origin: Pos2,
        ...
        let texture = if response.is_pointer_button_down_on() {
            if down {
                self.tex.switch_down.as_ref()
            } else {
                self.tex.switch_up.as_ref()
            }
        } else {
    ...
    fn draw_inside_text(&self, ui: &mut egui::Ui, drawer: Rect, scale: f32) {
        ...
        painter.text(Pos2::new(x, y), egui::Align2::LEFT_TOP, "REGISTERS", mono(14.0), ink);
        painter.text(
        ...
        let memory = &self.machine.bus.memory;
        let op = memory.get(pc).copied().unwrap_or(0);
        let b1 = memory.get(pc.wrapping_add(1)).copied().unwrap_or(0);
        let b2 = memory.get(pc.wrapping_add(2)).copied().unwrap_or(0);
        painter.text(
        ...
        for row in 0..7 {
            ...
            if address >= machine::MEM_SIZE {
                break;
            }
            let mut line = format!("{address:04X}   ");
            for col in 0..16 {
                if let Some(value) = memory.get(address + col) {
    ...
    fn draw_altair(&mut self, ui: &mut egui::Ui) {
        ...
        let drawer = Rect::from_min_size(
            origin + Vec2::new(DRAWER_X * scale, drawer_y * scale),
            Vec2::new(DRAWER_W * scale, DRAWER_H * scale),
        );
        if let Some(texture) = &self.tex.slideout {
            Self::image(ui, texture, drawer);
        } else {
            ui.painter().rect_filled(drawer, 8.0, Color32::from_rgb(74, 76, 74));
        }
        self.draw_inside_text(ui, drawer, scale);

        if let Some(texture) = &self.tex.panel {
            Self::image(ui, texture, panel_rect);
        } else {
            ui.painter().rect_filled(panel_rect, 0.0, Color32::from_rgb(25, 35, 43));
        }

        for bit in 0..16 {
            let switch_rect = Rect::from_min_size(
                origin + Vec2::new(SWITCH_X[bit] * scale, SWITCH_Y[bit] * scale),
                Vec2::new(32.0 * scale, 96.0 * scale),
            );
            let response = ui.allocate_rect(switch_rect, Sense::click());
            if response.clicked() {
                self.machine.bus.panel_switches ^= 1u16 << bit;
                self.audio.play_once("assets/click.mp3");
            }
            let up = self.machine.bus.panel_switches & (1u16 << bit) != 0;
            if let Some(texture) = if up {
                self.tex.switch_up.as_ref()
            } else {
                self.tex.switch_down.as_ref()
            } {
                Self::image(ui, texture, switch_rect);
            }
        }
        ...
        self.led(ui, origin, scale, 218.0, 228.0, self.machine.wait_led);
        // PROT is the LED immediately to the left of MEMR on the photographed panel.
        self.led(
            ui,
            origin,
            scale,
        ...
        self.led(ui, origin, scale, 324.0, 119.0, self.machine.powered);
        self.led(ui, origin, scale, 434.0, 120.0, self.machine.powered);
        self.led(ui, origin, scale, 654.0, 120.0, self.machine.powered);

        let power_rect = Rect::from_min_size(
            origin + Vec2::new(114.0 * scale, 408.0 * scale),
            Vec2::new(32.0 * scale, 96.0 * scale),
        );
        ...
        if let Some(texture) = if self.machine.powered {
            self.tex.switch_down.as_ref()
        } else {
            self.tex.switch_up.as_ref()
        } {
            Self::image(ui, texture, power_rect);
        }

        if let Some(down) = self.momentary(ui, origin, scale, 377.0, 410.0, "RUN / STOP") {
            self.machine.set_running(down);
        }
        if self
            .momentary(ui, origin, scale, 486.0, 410.0, "SINGLE STEP")
            .is_some()
        {
            self.machine.step();
        }
        if let Some(down) = self.momentary(
            ui,
            origin,
            scale,
            595.0,
        ...
        if self
            .momentary(ui, origin, scale, 813.0, 410.0, "RESET")
            .is_some()
        {
            self.machine.reset();
        ...
        // Unlike the S2JS page, these original front-panel switches are all
        // represented. PROTECT is functional; both AUX switches are intentionally
        // unassigned on a base Altair and therefore only provide mechanical action.
        if let Some(down) = self.momentary(
            ui,
            origin,
            scale,
        ...
        let _ = self.momentary(ui, origin, scale, 1031.0, 412.0, "AUX 1 (unassigned)");
        let _ = self.momentary(ui, origin, scale, 1140.0, 412.0, "AUX 2 (unassigned)");

        let handle = Rect::from_min_size(
            drawer.min + Vec2::new(450.0 * scale, 178.0 * scale),
            Vec2::new(205.0 * scale, 57.0 * scale),
        );
        let handle_response = ui.allocate_rect(handle, Sense::click());
        if handle_response.clicked() && self.machine.powered {
            self.inside_open = !self.inside_open;
            self.audio.play_once("assets/click.mp3");
        }
        if handle_response.hovered() {
            handle_response.on_hover_text(if self.inside_open {
    ...
    /// Drain characters written by SIO/SIO2 regardless of whether the ASR window
    /// currently has keyboard focus. The original web version automatically
    /// switches an OFF ASR-33 to LINE when the Altair sends its first character.
    fn process_tty_serial(&mut self) {
        while let Some(byte) = self.machine.bus.serial_tx.pop_front() {
    ...
    fn process_tty_keyboard(&mut self, ctx: &egui::Context) {
        ...
        ctx.input(|input| {
            for event in &input.events {
                match event {
                    egui::Event::Text(s) => {
                        bytes.extend(s.bytes().map(|b| b.to_ascii_uppercase()));
                    }
    ...
    fn update_key_animation(&mut self, ctx: &egui::Context) {
        let now = Instant::now();
        let dt = now
            .duration_since(self.key_anim_tick)
            .as_secs_f32()
            .min(0.05);
    ...
    fn press_tty_key(&mut self, index: usize) {
        if self.tty.mode == TtyMode::Off || self.pressed_key.is_some() {
            return;
        }
        ...
        match key.kind {
            ...
            kind => {
                if let Some(byte) =
                    teletype::key_to_byte(kind, self.tty.shift_down, self.tty.control_down)
                {
                    self.send_tty_byte(byte);
                }
            }
    ...
    fn draw_pressed_key(&self, ui: &mut egui::Ui, origin: Pos2, scale: f32) {
        let Some(index) = self.animated_key else {
            return;
        };
        if self.key_displacement <= 0.0 {
            return;
    ...
    fn draw_teletype(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some(texture) = &self.tex.tty_body {
            Self::image(ui, texture, rect);
        } else {
            ui.painter().rect_filled(rect, 0.0, Color32::from_rgb(80, 76, 65));
        }
        if let Some(texture) = &self.tex.tty_keys {
            Self::image(ui, texture, rect);
        }

        let paper = Rect::from_min_size(
            origin + Vec2::new(teletype::PRINT_LEFT * scale, teletype::PRINT_TOP * scale),
            Vec2::new(teletype::PRINTABLE_WIDTH * scale, TTY_H * 0.27 * scale),
        );
        let char_width = self.tty.char_width_image_px();
        let font_size = (char_width * 1.63 * scale).max(5.0);
        let family = FontFamily::Name("teletype".into());
        ui.painter().text(
            paper.left_bottom(),
        ...
        if self.tty.mode != TtyMode::Off {
            if let Some(texture) = &self.tex.tty_head {
                let x = teletype::PRINT_LEFT
                    + self.tty.column as f32 * char_width
                    - char_width;
                let head_rect = Rect::from_min_size(
                    origin + Vec2::new(x * scale, teletype::PRINT_HEAD_TOP * scale),
                    Vec2::new(TTY_W * 0.06 * scale, TTY_H * 0.10 * scale),
                );
                Self::image(ui, texture, head_rect);
            }
        }
        ...
        if let Some(texture) = &self.tex.tty_line_local {
            Self::image(ui, texture, selector);
        }

        if response.drag_started()
            || (response.is_pointer_button_down_on() && self.pressed_key.is_none())
        {
            if let Some(pointer) = response.interact_pointer_pos() {
                if selector.contains(pointer) {
                    ...
                    let image_x = (pointer.x - rect.left()) / scale;
                    let image_y = (pointer.y - rect.top()) / scale;
                    if let Some(index) = teletype::KEYS
                        .iter()
                        .position(|key| key.contains(image_x, image_y))
                    {
                        self.press_tty_key(index);
                    }
                }
            }
        }
        ...
        if let Some(texture) = &self.tex.tty_knob {
            let knob_w = TTY_W * 0.06 * scale;
            let knob_h = knob_w * 117.0 / 116.0;
            let base = rect.right() - TTY_W * 0.06 * scale - knob_w;
            let shift = match self.tty.mode {
                TtyMode::Line => -0.35 * knob_w,
                TtyMode::Off => 0.0,
                TtyMode::Local => 0.35 * knob_w,
            };
            ...
            Self::image(ui, texture, knob_rect);
        }
    }

    fn update_paper_tape(&mut self) {
        if self.last_tape_tick.elapsed() < Duration::from_millis(30) {
            return;
        }
    ...
    fn draw_tty_window(&mut self, ctx: &egui::Context) {
        ...
        egui::TopBottomPanel::bottom("tty-status").show(ctx, |ui| {
            ui.small("ASR-33 connected to Altair SIO/SIO2 ports 00/01 and 10/11");
        });
    }

    fn show_tty_viewport(&mut self, parent_ctx: &egui::Context) {
        if !self.tty_window_open {
            return;
        }
...
impl eframe::App for RusTairApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.machine.running {
            // Keep CPU timing based on elapsed real time, but do not render as fast
            // as the event loop permits. At the 20 ms dt cap, 2 MHz needs at most
            // 40,000 cycles in one update, so this preserves clock rate without
            // the old 10,000-cycle throttle.
            let cycles = (CLOCK_HZ as f64 * dt.as_secs_f64()) as u32;
            self.machine.run_cycles(cycles.clamp(1, 100_000));
            ctx.request_repaint_after(PANEL_FRAME);
        }

        self.process_tty_serial();
