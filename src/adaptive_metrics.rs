...
impl AdaptiveCycleStats {
    ...
    pub fn full_percent(self) -> f64 {
        ...
        if total == 0 {
            ...
            self.full_t_states as f64 * 100.0 / total as f64
        }
    }
    ...
    pub fn partial_percent(self) -> f64 {
        ...
        if total == 0 {
            ...
            self.partial_t_states as f64 * 100.0 / total as f64
        }
    }
}
...
pub(crate) fn record_full_window(instructions: u64, t_states: u64) {
    ...
    METRICS.with(|metrics| {
        ...
        metrics.stats.full_instructions =
            metrics.stats.full_instructions.saturating_add(instructions);
    ...
    METRICS.with(|metrics| {
        ...
        match reason {
            AdaptiveFallbackReason::ChassisUnsupported => {
                metrics.stats.fallbacks.chassis_unsupported = metrics
                    .stats
                    .fallbacks
                    .chassis_unsupported
                    .saturating_add(1)
            ...
            AdaptiveFallbackReason::ReadyLow => {
                metrics.stats.fallbacks.ready_low =
                    metrics.stats.fallbacks.ready_low.saturating_add(1)
            }
            ...
            AdaptiveFallbackReason::InterruptPending => {
                metrics.stats.fallbacks.interrupt_pending =
                    metrics.stats.fallbacks.interrupt_pending.saturating_add(1)
            }
            AdaptiveFallbackReason::BudgetTail => {
                metrics.stats.fallbacks.budget_tail =
                    metrics.stats.fallbacks.budget_tail.saturating_add(1)
            }
            AdaptiveFallbackReason::NotInstructionBoundary => {
                metrics.stats.fallbacks.not_instruction_boundary = metrics
                    .stats
                    .fallbacks
                    .not_instruction_boundary
                    .saturating_add(1)
            }
            AdaptiveFallbackReason::StopWaitPending => {
                metrics.stats.fallbacks.stop_wait_pending =
                    metrics.stats.fallbacks.stop_wait_pending.saturating_add(1)
            }
            AdaptiveFallbackReason::CpuFault => {
                metrics.stats.fallbacks.cpu_fault =
                    metrics.stats.fallbacks.cpu_fault.saturating_add(1)
            }
            ...
            AdaptiveFallbackReason::OpcodeBarrier => {
                metrics.stats.fallbacks.opcode_barrier =
                    metrics.stats.fallbacks.opcode_barrier.saturating_add(1)
            }
            AdaptiveFallbackReason::FullWindowUnavailable => {
                metrics.stats.fallbacks.full_window_unavailable = metrics
                    .stats
                    .fallbacks
                    .full_window_unavailable
                    .saturating_add(1)
            }
        }
