...
- [ ] **Phase 3 — FD-400 mechanics/time engine:** epoch/deadline rotation, hard-sector position, track/head/step mechanics and chassis-time integration are implemented; installed-idle DCDD is benchmarked below the 2% budget with no global pre-Phase3 regression. Remaining before PASS: source-exact ~5 s drive-ready gating after power/door close, guest-visible TRACK0/Head Status/sector-position mechanics, stopped-CPU chassis-clock integration evidence, and exact Move Head timing without approximation.
...
- [ ] **Phase 11 — performance/closeout:** add installed-idle, polling, continuous read/write, late-byte and many-idle-drive benchmarks; installed-idle evidence currently meets the <2% target, while active polling/transfer budgets remain for the phases that implement those paths.
