...
Implementation contract: [`docs/88_DCDD_FD400_IMPLEMENTATION_PLAN.md`](docs/88_DCDD_FD400_IMPLEMENTATION_PLAN.md). The authentic and accelerated disk paths must share the same physical controller/drive state machines; Fast Disk removes waits but must never become a guest-visible sector API or be implied by host `Unlimited` execution speed.

- [ ] **Phase 0 — source-backed hardware contract:** lock down ports, polarities, controller-board wiring, drive signals, timing, interrupts, addressing, revisions and the first supported media representation from primary hardware documentation before encoding assumptions in production code.
