...
function Replace-Exactly {
    ...
    if ($matches -eq 0) {
        ...
        throw "Expected source anchor not found in $Path"
    }
    if ($matches -ne 1) {
        throw "Expected exactly one source anchor in $Path, found $matches"
    }
...
$statePath = 'src/cpu8080_cycle/state.rs'
$stateOld = @'
...
$stateNew = @'
                ...
                | Instruction::Sphl
'@
Replace-Exactly -Path $statePath -Old $stateOld -New $stateNew

$fullPath = 'src/backend/cycle/full.rs'
$fullOld = @'
...
$fullNew = @'
    ...
    fn project_opcode_fetch(&mut self, address: u16, opcode: u8) {
        self.project_machine_cycle(address, opcode, 0xa2, 4, true, false);
        // The instruction-level semantic core reports only the total T-count;
        // its Bus callbacks expose external transfers. These families extend
        // M1 through T5 before any following operand/stack transfer, so Full
        // must materialize that internal T-state here rather than as a residual
        // after the instruction's final external cycle.
...
'@
Replace-Exactly -Path $fullPath -Old $fullOld -New $fullNew

$testName = 'fn compiled_full_control_flow_t5_families_match_forced_partial_exactly()'
$fullText = [System.IO.File]::ReadAllText($fullPath)
if (-not $fullText.Contains($testName)) {
    $marker = @'
    #[test]
...
'@
    ...
    fn compiled_full_control_flow_t5_families_match_forced_partial_exactly() {
        ...
        // Representative taken/not-taken control-flow cases plus every
        // undocumented unconditional CALL alias. All of these families keep M1
        // alive for T5 before any later operand/stack machine cycle.
        for (opcode, flags, expected_full_t, name) in [
            ...
            // Taken Rcc consumes an existing return address from the stack.
            for backend in [&mut compiled, &mut partial] {
                backend.machine.bus.debugger_write_memory(0x0800, 0x08, false);
                backend.machine.bus.debugger_write_memory(0x0801, 0x00, false);
            }
            ...
            assert_eq!(
                ...
                u64::from(BUDGET - expected_full_t as u32),
                "{name} budget tail"
            );
            assert_eq!(
                stats.fallbacks.opcode_barrier,
                0,
                "{name} must not be a barrier"
            );
            ...
            assert_eq!(
                ...
                "{name} final status latch"
            );
            assert_eq!(
                compiled.machine.bus.raw_panel_data(),
                partial.machine.bus.raw_panel_data(),
                "{name} final panel DATA"
            );
        }
    }

'@
    $markerCount = [regex]::Matches($fullText, [regex]::Escape($marker)).Count
    if ($markerCount -ne 1) {
        throw "Expected exactly one test insertion marker in $fullPath, found $markerCount"
    }
    $fullText = $fullText.Replace($marker, $testBlock + $marker)
    [System.IO.File]::WriteAllText(
        $fullPath,
        $fullText,
        [System.Text.UTF8Encoding]::new($false)
    )
    Write-Host "Added control-flow Full/Partial oracle test"
} else {
    Write-Host 'Control-flow oracle test already present'
}

cargo fmt --all
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host 'Full control-flow barrier changes applied and formatted.'
